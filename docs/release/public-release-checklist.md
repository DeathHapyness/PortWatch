...
- [ ] Licença MIT e arquivos comunitários integrados na `main`.
- [ ] Token do frontend informado em runtime, sem segredo no bundle.
...
- [ ] Referência de todas as variáveis de ambiente, defaults e limites.
- [ ] TLS/reverse proxy e rotação do token documentados.
- [ ] Suíte backend, frontend, Netprobe, políticas de Compose e E2E verdes.
- [ ] Teste real confirma que Uvicorn/proxy não divulga versão no `Server`.
